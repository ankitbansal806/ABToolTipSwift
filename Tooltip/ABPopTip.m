...
#import "ABPopTip.h"
#import "ABPopTipDefaults.h"
#import "ABPopTip+Draw.h"
...
@property (nonatomic, assign, readwrite) ABPopTipDirection direction;
...
@implementation ABPopTip

+ (instancetype)popTip {
    return [[ABPopTip alloc] init];
}
...
- (void)commonInit {
    ...
    _entranceAnimation = ABPopTipEntranceAnimationScale;
    _actionAnimation = ABPopTipActionAnimationNone;
...
- (void)setup {
    if (self.direction == ABPopTipDirectionLeft) {
    ...
    if (self.direction == ABPopTipDirectionRight) {
    ...
    float offset = self.offset * ((self.direction == ABPopTipDirectionUp || self.direction == ABPopTipDirectionLeft || self.direction == ABPopTipDirectionNone) ? -1 : 1);

    if (self.direction == ABPopTipDirectionUp || self.direction == ABPopTipDirectionDown) {
        ...
        if (self.direction == ABPopTipDirectionDown) {
    ...
    } else if (self.direction == ABPopTipDirectionLeft || self.direction == ABPopTipDirectionRight) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.isRounded) {
        BOOL showHorizontally = self.direction == ABPopTipDirectionLeft || self.direction == ABPopTipDirectionRight;
...
- (void)show {
    ...
    [self performEntranceAnimation:^{
        ...
        if (self.actionAnimation != ABPopTipActionAnimationNone) {
            [self startActionAnimation];
        }
    }];
}

- (void)showText:(NSString *)text direction:(ABPopTipDirection)direction maxWidth:(CGFloat)maxWidth inView:(UIView *)view fromFrame:(CGRect)frame {
...
- (void)showAttributedText:(NSAttributedString *)text direction:(ABPopTipDirection)direction maxWidth:(CGFloat)maxWidth inView:(UIView *)view fromFrame:(CGRect)frame {
    self.text = nil;
...
- (void)showText:(NSString *)text direction:(ABPopTipDirection)direction maxWidth:(CGFloat)maxWidth inView:(UIView *)view fromFrame:(CGRect)frame duration:(NSTimeInterval)interval {
...
- (void)showAttributedText:(NSAttributedString *)text direction:(ABPopTipDirection)direction maxWidth:(CGFloat)maxWidth inView:(UIView *)view fromFrame:(CGRect)frame duration:(NSTimeInterval)interval {
